...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if ProcessInfo.processInfo.environment["CODENOTCH_DEMO"] == "1" {
            ...
            // Comparison edition starts with its own preferences.
        ...
        preferences?.$disconnectedProviders
            .receive(on: RunLoop.main)
            .sink { [weak self, weak controller] disconnected in
                guard let self else { return }
                for (id, monitor) in self.monitors {
                    if disconnected.contains(id) {
                        if self.activeMonitors.remove(id) != nil { monitor.stop() }
                        controller?.model.sessions[id] = []
                    } else if self.activeMonitors.insert(id).inserted {
                        monitor.start()
                    }
                }
            }
