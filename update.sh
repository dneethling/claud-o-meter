...
"$WIDGET_DIR/.venv/bin/python" "$WIDGET_DIR/widget_update.py" install
RC=$?
if [ "$RC" -eq 0 ]; then
  osascript -e 'display notification "Update installed. Refresh the widget to use it." with title "Claud-o-meter"' 2>/dev/null || true
elif [ "$RC" -eq 3 ]; then
  osascript -e 'display notification "An update is already running. Check the widget menu shortly." with title "Claud-o-meter"' 2>/dev/null || true
else
